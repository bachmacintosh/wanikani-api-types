...
describe("Revision 20170710: Base", () => {
  test("ApiRevision", () => {
    ...
    expect(() => parse(ApiRevision, apiRevision)).not.toThrow();
  });
  test("DatableString", () => {
    ...
    expect(() => parse(DatableString, dateTimeUtcString)).not.toThrow();
    expect(() => parse(DatableString, dateTimeOffsetString)).not.toThrow();
    expect(() => parse(DatableString, validLeapYear)).not.toThrow();
    expect(() => parse(DatableString, dateIsoString)).not.toThrow();
  });
});
